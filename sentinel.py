...
BACKEND_CONTAINER_MAP = {
    ...
    "make": None,  # External service - Cloudflare Workers
...
async def restart_container(container_name: str) -> Dict:
    """Restart an Azure Container App using Azure CLI."""
    import subprocess
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Try updating min-replicas as fallback
            cmd2 = [
...
async def log_to_hive_mind(session: aiohttp.ClientSession, category: str, summary: str, 
                           details: Dict, priority: str = "MEDIUM", tags: List[str] = None) -> bool:
    """Log an entry to Hive Mind via Snowflake MCP."""
    ...
    (CREATED_AT, CATEGORY, SUMMARY, DETAILS, PRIORITY, SOURCE, WORKSTREAM, TAGS)
    VALUES (
        '{timestamp}',
        '{category}',
        '{summary.replace("'", "''")}',
...
async def call_for_help(session: aiohttp.ClientSession, issue: str, details: Dict) -> None:
    """Escalate to Hive Mind when auto-remediation fails."""
    await log_to_hive_mind(
        session,
        category="ALERT",
        summary=f"🚨 SENTINEL NEEDS HELP: {issue}",
        details={
            **details,
        ...
        priority="HIGH",
        tags=["sentinel", "escalation", "needs-help", "lb-mcp"]
    )
...
async def remediate_backend(session: aiohttp.ClientSession, backend_name: str, error: str) -> bool:
    """Attempt to fix a failing backend."""
    ...
    if result.get("success"):
        await log_to_hive_mind(
            ...
            summary=f"✅ Auto-remediated {backend_name} by {result.get('action', 'restart')}",
            details={
            ...
            priority="MEDIUM",
            tags=["sentinel", "remediation", "success"]
        )
        return True
    else:
        await call_for_help(
            session,
            f"Failed to remediate {backend_name}",
            {
                "backend": backend_name,
                "container": container,
                "original_error": error,
                "remediation_error": result.get("error")
            }
        )
        return False
...
async def keep_connections_alive(session: aiohttp.ClientSession, health_data: Dict) -> None:
    """Send keepalive pings to all healthy backends."""
    backends = health_data.get("backends", {})
    
    for name, info in backends.items():
        if info.get("status") == "healthy":
            # Just log - the health check itself serves as keepalive
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if lb_health["status"] == "healthy":
            ...
            issues.append({
                "gateway": "lb_mcp",
                "backend": "gateway",
                "status": lb_health["status"],
                "error": lb_health["error"]
            })
        
        # Check Gateway V3 (backup)
        if v3_health["status"] != "healthy":
            issues.append({
                "gateway": "v3",
                "backend": "gateway",
                "status": v3_health["status"],
                "error": v3_health["error"]
            })
        ...
        if issues:
            await log_to_hive_mind(
                ...
                summary=f"⚠️ LB MCP: {healthy_count}/{total_count} backends healthy, {len(issues)} issues",
                details={
                    "timestamp": timestamp,
                    "lb_mcp_tools": lb_health.get("total_tools", 0),
                    "v3_tools": v3_health.get("total_tools", 0),
                    "issues": issues,
                    "remediations": remediations
                },
                priority="HIGH" if len(issues) > 3 else "MEDIUM",
                tags=["sentinel", "health-check", "issues"]
            )
        else:
            await log_to_hive_mind(
                session,
                category="HEALTH_CHECK",
                summary=f"✅ LB MCP healthy: {lb_health.get('total_tools', 0)} tools, V3 backup: {v3_health.get('total_tools', 0)} tools",
                details={
                    "timestamp": timestamp,
                    "lb_mcp_tools": lb_health.get("total_tools", 0),
                    "v3_tools": v3_health.get("total_tools", 0),
                    "lb_backends": healthy_count,
                    "v3_backends": len(v3_health.get("backends", {}))
                },
                priority="LOW",
                tags=["sentinel", "health-check", "healthy"]
            )
        
        return {
            "timestamp": timestamp,
            "lb_mcp": lb_health,
            "gateway_v3": v3_health,
            "issues": issues,
            "remediations": remediations
        }
...
async def main():
    ...
    while True:
        try:
            ...
            logger.info(
                f"🔍 Check complete: LB {lb_status} ({result['lb_mcp'].get('total_tools', 0)} tools) | "
                f"V3 {v3_status} ({result['gateway_v3'].get('total_tools', 0)} tools) | "
